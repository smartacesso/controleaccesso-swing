...
public class TopDataDevice extends Device {
	...
	public void connect(String... args) throws Exception {
		...
		nivelSeguranca = getConfigurationValueAsInteger("N�vel de seguran�a do reconhecimento");
		if (nivelSeguranca == 0) {
			nivelSeguranca = 6;
		}
...
//												|| inner.TipoLeitor ==  Enumeradores.CODIGO_BARRAS_SERIAL
											...
											Cartao = new StringBuffer(Utils.conversorHexaDecimal(Cartao.toString().replaceAll("[^a-zA-Z0-9]+","")));
										}
									}
								}
	...
	private void enviarDigitaisLFD(boolean online, boolean todas, Date data) {
		...
		try {
			...
			if(pedestres != null && !pedestres.isEmpty()) {
				...
				//indica atualiza��o feita quando
	...
	protected void registraGiro(int sentido, Date data) {
		...
		if(inner.BilheteInner.Cartao != null
			...
			if(this.matchedFacialId != null) {
				...
				//retorna, pois foi um giro de libera��o manual
...
//		enviarConfiguracoesOffline();
		...
		if(Boolean.TRUE.equals(getConfigurationValueAsBoolean("Coleta cart�es offline")))
...
/*		if (ret != easyInner.RET_COMANDO_OK) {
			...
			Main.mainScreen.addEvento(" N�o foi poss�vel enviar a lista de cart�es offline");
		}
*/
	}
...
//			templateEntity.setSample(new byte[10]);
	...
	protected void configureInner(){
		...
		this.inner.Identificacao = getConfigurationValueAsInteger("identifica��o Biom�trica");
		this.inner.Verificacao = getConfigurationValueAsInteger("Verifica��o Biom�trica");
...
//			no validar acesso temos a configura��es de colocar Access date,
...
//>>>>>>> 9f4c8fd93a3f38e68c373434cc20a2262192bb2c
	...
	protected void enviarConfiguracaoMudancaOnlineOffline() throws Exception{
		int countTentativasEnvioComando = 0;
    	int tempoMudancaOnlineOffline = getConfigurationValueAsInteger("Tempo de mudan�a Online/Offline");
	...
	protected void montaConfiguracaoInner(int modo) {
		try {
			...
			int tempoLiberacao = getConfigurationValueAsInteger("Tempo de libera��o");
			tempoLiberacao = 5;
