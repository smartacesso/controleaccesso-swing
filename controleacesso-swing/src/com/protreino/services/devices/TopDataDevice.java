...
public class TopDataDevice extends Device {
	...
	// private int exportType = NBioBSPJNI.EXPORT_MINCONV_TYPE.FIM01_HV; // FIM01_HV
	// = 7 ; ANSI = 35
	...
	public void connect(String... args) throws Exception {
		...
		if (!(this instanceof TopDataExpedidoraDevice) && Boolean.TRUE.equals(onlyEnabledMode)) {
			...
			if (ret == Enumeradores.RET_COMANDO_OK) {
				...
				throw new Exception("Catraca " + deviceEntity.getName() + " nao pode ser conectada");
			}
				...
				while (workerEnabled) {
					try {
						...
						if (DeviceMode.VERIFICATION.equals(mode)) {
							...
							if (Cartao.toString().startsWith("05000") || Cartao.toString().startsWith("005000")
							...
							if (inner.TipoLeitor == Enumeradores.QRCODE
								...
								if (ret == Enumeradores.RET_COMANDO_OK && !Cartao.toString().contains("_")) {
									if ((Cartao != null && Cartao.length() != 0) || iArrBCartaoRb[0] == 2
											|| iArrBCartaoRb[0] == 3) {
										Cartao = new StringBuffer(
												Utils.toHEX(Cartao.toString().replaceAll("[^a-zA-Z0-9]+", "")));
									}
								}
								...
								try {
									...
									if (iArrBCartaoRb[0] == Enumeradores.GIRO_DA_CATRACA_TOPDATA) {
										...
										allowAccess();
											...
											|| iArrBCartaoRb[0] == Enumeradores.TECLA_FUNCAO
											|| iArrBCartaoRb[0] == Enumeradores.TECLA_ANULA || ((Cartao.length() == 0)
													&& !(inner.EstadoTeclado == Enumeradores.EstadosTeclado.AGUARDANDO_TECLADO))) {
										enviarMensagemPadrao();
										configurarEntradasOnline();
							...
							if (modeloLC) {
								...
								do {
									ret = EasyInner.RespostaReceberTemplateLeitorInnerBio(inner.Numero,
											new int[] { 502 });
									Thread.sleep(500);
								} while (ret != Enumeradores.RET_COMANDO_OK && DeviceMode.ENROLLMENT.equals(mode));
								...
								if (ret == Enumeradores.RET_COMANDO_OK) {
									...
									do {
										ret = EasyInner.ReceberTemplateLeitorInnerBio(inner.Numero, templateTemp, 502);
										Thread.sleep(500);
									} while (ret != Enumeradores.RET_COMANDO_OK && templateTemp != null
											&& (System.currentTimeMillis() - inicio) < 5000
											&& DeviceMode.ENROLLMENT.equals(mode));
						...
						if (System.currentTimeMillis() - lastResponseTime > MAX_TIME_WITHOUT_RESPONSE) {
							// Se o tempo sem resposta for maior que o limite, verifica o cooldown de
							// reinicialização
				...
				while (watchDogEnabled) {
					...
					try {
						...
						if (DeviceStatus.DISCONNECTED.equals(getStatus())) {
							System.out.println(
									Main.sdf.format(new Date()) + " Catraca desconectada, resposta do ping: " + ret);
						}

						if (DeviceStatus.DISCONNECTED.equals(lastStatus)
								&& DeviceStatus.CONNECTED.equals(getStatus())) {
							System.out.println(
									Main.sdf.format(new Date()) + " Catraca reconectada, resposta do ping: " + ret);
...
//								public void run() {
							try {
								System.out.println(Main.sdf.format(new Date())
										+ " Desconectando catraca, pode levar ate 5 segundos");
								disconnect();
								Utils.sleep(5000);
								...
								System.out.println(
										Main.sdf.format(new Date()) + " Erro ao reconectar a catraca, veja abaixo: ");
								e.printStackTrace();
								if (watchDog != null && !watchDogEnabled) {
									System.out.println(Main.sdf.format(new Date())
											+ " Reiniciando processo de reconexão automatica... ");
									watchDogEnabled = true;
									watchDog.execute();
								}
							}
...
//							}.start();
	...
	private void enviarDigitaisLFD(boolean online, boolean todas, Date data) {
		...
		try {
			...
			if (!todas && dataAlteracao != null) {
				System.out.println("Sincroniza por data "
						+ new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(deviceEntity.getUltimaAtualizacao()));

				HashMap<String, Object> args = new HashMap<String, Object>();
				args.put("ULTIMA_SINC", dataAlteracao);
				pedestres = (List<PedestrianAccessEntity>) HibernateAccessDataFacade.getResultListWithParams(
						PedestrianAccessEntity.class, "PedestrianAccessEntity.findAllPedestreAlterados", args);

			} else {
				System.out.println("Sincroniza total");
				pedestres = (List<PedestrianAccessEntity>) HibernateAccessDataFacade
						.getResultList(PedestrianAccessEntity.class, "PedestrianAccessEntity.findAllPedestre");
			}
			...
			for (PedestrianAccessEntity pedestre : pedestres) {
				...
				if (!alteraTemplate && dataAlteracao != null
						&& templates.get(0).getDataCriacao().getTime() >= dataAlteracao.getTime()) {
					alteraTemplate = true;
				}

				if (!alteraTemplate && dataAlteracao != null && templates.size() >= 2
				...
				if (alteraTemplate) {
					...
					System.out.println("Biometrias Nao alteradas " + pedestre.getName());
				}
	...
	private void manutencaoDigitalCatraca(boolean online, TemplateEntity t) {
		...
		if (ret == Enumeradores.RET_COMANDO_OK) {
			...
			if (ret == Enumeradores.RET_COMANDO_OK || ret == 1) {
				System.out.println("Usuario enviado catraca ID: " + t.getPedestrianAccess().getId());
			} else {
				System.out.println("Usuario Nao enviado catraca, motivo: " + ret);
			}
	...
	private byte[] montaBytesCatraca(TemplateEntity t) {
		...
		// 0 Master (0 = Nao / 1 = sim) sempre utilizar 0
		template[0] = 0;

		// 1 - 10 Número do usuario (em ASC)
		int p = 1;
		...
		for (byte b : (t.getSample() != null ? t.getSample() : t.getTemplate())) {
			template[p] = b;
			p++;
		}
		...
		template[p] = (byte) Integer.parseInt(dataCad.get("yyyy").toString().substring(0, 2));
		p++;
		template[p] = (byte) ((int) dataCad.get("yyyy") % 100);
		p++;
		template[p] = (byte) (int) dataCad.get("MM");
		p++;
		template[p] = (byte) (int) dataCad.get("dd");
		p++;
		template[p] = (byte) (int) dataCad.get("HH");
		p++;
		template[p] = (byte) (int) dataCad.get("mm");
		p++;
		template[p] = (byte) (int) dataCad.get("ss");
		p++;
		template[p] = 0;
	...
	private boolean removeDigitalLFD(boolean online, PedestrianAccessEntity p) {
		...
		if (ret == Enumeradores.RET_COMANDO_OK) {
			...
			if (ret != Enumeradores.RET_COMANDO_OK && ret != 1 && ret != Enumeradores.RET_BIO_USR_NAO_CADASTRADO) {
				System.out.println("Nao encontrado na catraca (verifica removido): " + ret);
				return false;
			}
			...
			if (Main.desenvolvimento) {
				System.out.println("Nao encontrado na catraca (solicita removido): " + ret);
			}
			return false;
		}
	...
	public void sendConfiguration() throws Exception {
		...
		if (modeloLC) {
			...
			EasyInner.ConfigurarComportamentoIndexSearch(
					"naCatraca".equals(getConfigurationValue(MODO_DE_TRABALHO)) ? 0 : 1); // 0=match na catraca 1=match
																							// no computador
		}
...
//            	bilhetes != null && bilhetes[0] > 0
				if (true) {
							...
							if (Ret == 0) {
								...
								if (!"".equals(valores.toString())) {
									...
									if (VerificationResult.ALLOWED.equals(this.verificationResult)) {
										...
										int sentido = 0;
										if (this instanceof TopDataExpedidoraDevice && this.tipo.equals(Tipo.ENTRADA)) {
									...
									System.out.println("cartao vazio!");
								}
							} else {
								System.out.println("Nao foi possivel coletar os bilhetes na catraca: " + Ret);
							}
						} catch (Throwable e) {
							e.printStackTrace();
						}
					...
					if (qtdPermitido > 0 || qtdNaoPermitido > 0) {
						Utils.createNotification("Dados recuperados da catraca: " + (qtdNaoPermitido + qtdPermitido),
	...
	private void definiMensagemExibidaNoDisplay() {
		...
		if (VerificationResult.ALLOWED.equals(verificationResult)
				&& (messagePersonalizedInDevice == null || messagePersonalizedInDevice.isEmpty())) {
	...
	private int decideLadoEntrada(String sentidoCatraca, boolean bloquearSaida) {
		...
		if (bloquearSaida && matchedAthleteAccess != null) {
			if (inner.BilheteInner.Origem == Enumeradores.ORIGEM_URNA
					|| inner.BilheteInner.Origem == Origens.ORIGEM_LEITOR_2) {
				...
				if (doisSentidosLiberado) {
					...
					ret = !"anticlockwise".equals(sentidoCatraca) ? EasyInner.LiberarCatracaEntrada(inner.Numero)
			...
			LogPedestrianAccessEntity lastAccess = logPedestrianAccessRepository
					.buscaUltimoAcesso(matchedAthleteAccess.getId(), matchedAthleteAccess.getQtdAcessoAntesSinc());
			if (lastAccess == null || Tipo.SAIDA.equals(lastAccess.getDirection())
					|| lastAccess.getDirection() == null) {
			...
			if (ReleaseAccessUseCase.getApertouF10()) {
				...
				if (doisSentidosLiberado) {
					...
					return !"anticlockwise".equals(sentidoCatraca) ? EasyInner.LiberarCatracaEntrada(inner.Numero)
							: EasyInner.LiberarCatracaEntradaInvertida(inner.Numero);
				}
	...
	public void denyAccess() {
		try {
			...
			if (ret == Enumeradores.RET_COMANDO_OK) {
				...
				Main.mainScreen.addEvento(name + ": Nao foi possivel enviar mensagem acesso negado");
				setStatus(DeviceStatus.DISCONNECTED);
				throw new Exception(name + ": Nao foi possivel enviar mensagem acesso negado");
			}
		} catch (Exception ex) {
			ex.printStackTrace();
...
//		EasyInner.DefinirQuantidadeDigitosCartao(8);
		...
		List<PedestrianAccessEntity> biometriasNaoRemovidas = HibernateAccessDataFacade
				.buscaPedestresAtivosComBiometria();
		...
		if (samplesCollected >= 2) {
			...
			if (ret == Enumeradores.RET_COMANDO_OK) {
				EasyInner.EnviarMensagemPadraoOnLine(inner.Numero, 0,
						formatMessage(Utils.getPreference("messageEnrollmentFinished")));
			...
			EasyInner.EnviarMensagemPadraoOnLine(inner.Numero, 0, formatMessage(
					(Utils.getPreference("messageEnrollment") + " " + String.valueOf((samplesCollected + 1)) + "/2")));
		}
	...
	public void processSampleForEnrollment(Object obj) {
		try {
			...
			if (samplesCollected == 0) {
				...
				int ret = export.ImportFIR(templateTemp, 404, exportType, NBioBSPJNI.FIR_PURPOSE.ENROLL,
						storedFIRHandle);
				if (ret != Enumeradores.RET_COMANDO_OK) {
					Utils.createNotification("Ocorreu um erro ao processar amostra. Retorno: " + ret,
				...
				if (ret != Enumeradores.RET_COMANDO_OK) {
					Utils.createNotification("Ocorreu um erro ao criar o template. Retorno: " + ret,
							NotificationType.BAD);
					return;
				}
				storedInputFIR.SetFIRHandle(storedFIRHandle);
			...
			if (samplesCollected < 2) {
				EasyInner.EnviarMensagemPadraoOnLine(inner.Numero, 0,
						formatMessage((Utils.getPreference("messageEnrollment") + " "
	...
	public void processAccessRequest(Object obj) {
		try {
			Object[] retorno = processAccessRequestUseCase.processAccessRequest((String) obj, getFullIdentifier(),
					inner.BilheteInner.Origem, location, getConfigurationValueAsBoolean(LOGICA_DE_CATRACA_COM_URNA),
					true, getConfigurationValueAsBoolean(IGNORAR_REGRAS_DE_ACESSO));
	...
	public void processAccessRequest(Object obj, Date data) {
		try {
			Object[] retorno = processAccessRequestUseCase.processAccessRequest((String) obj, getFullIdentifier(),
					inner.BilheteInner.Origem, location, getConfigurationValueAsBoolean(LOGICA_DE_CATRACA_COM_URNA),
					false, data, isRegistrationProcessStartedOnDevice(),
					getConfigurationValueAsBoolean(IGNORAR_REGRAS_DE_ACESSO));
	...
	public Set<Integer> getRegisteredUserList() throws Exception {
		...
		System.out.println(sdf.format(new Date()) + "  TEMPO PARA RECEBER LISTA DE USUARIOS: "
				+ (System.currentTimeMillis() - inicio) + " ms");
		return retorno;
	}

	/*
	 * private String receberUsuarioDaCatraca(Long idUsuario) { // TODO int ret =
	 * EasyInner.SolicitarUsuarioCadastradoBio(inner.Numero, "0000000028"); if (ret
	 * != Enumeradores.RET_COMANDO_OK) return "Erro solicitacao " + ret;
	 * 
	 * System.out.println(sdf.format(new Date()) + "  Solicitado!");
	 * Utils.sleep(500);
	 * 
	 * Long inicio = System.currentTimeMillis(); byte[] template = new byte[844]; do
	 * { ret = EasyInner.ReceberUsuarioCadastradoBio(inner.Numero, 1, template);
	 * Utils.sleep(200); } while (ret != Enumeradores.RET_COMANDO_OK &&
	 * (System.currentTimeMillis()-inicio) < 20000);
	 * 
	...
	/**
	 * Método usado somente quando as digitais sao armazenadas no servidor. Realiza
	 * a coleta da digital para criar o template
	 */
	@Override
	public String cadastrateUser(PedestrianAccessEntity athleteAccessEntity) {
		...
		if (cancelAction) {
			...
			EasyInner.EnviarMensagemPadraoOnLine(inner.Numero, 0,
					formatMessage(Utils.getPreference("messageEnrollmentCancelled")));
			Utils.sleep(1000);
			setMode(DeviceMode.VERIFICATION);
			templates = null;
...
//			templateEntity.setSample(new byte[10]);
		...
		if (modeloLC) {
			...
			for (Device d : Main.devicesList) {
				if (d != null && d instanceof TopDataDevice && d.identifier != this.identifier) {
			...
			for (Device d : Main.devicesList) {
				if (d != null && d instanceof TopDataDevice) {
					// verifica se pode adicionar
					TopDataDevice topData = (TopDataDevice) d;
					if (this.innerNumber != topData.getInnerNumber() && topData.getIndexSearchEngine() != null) {
						System.out.println(sdf.format(new Date()) + "   Enviando template para indexSearch do Inner "
								+ topData.getInnerNumber() + "...");
	...
	private void removeBiometricaLC(PedestrianAccessEntity athleteAccessEntity, boolean isCatraca) {
		...
		// remove usuario em outras catracas
		if (isCatraca) {
	...
	public String setMode(DeviceMode mode) {
		...
		try {
			...
			return "Erro ao enviar as configuracoes. " + e.getMessage();
		}
		return "";
	}
	...
	public void createDefaultConfiguration() {
		...
		geralConfigurations.add(
				new ConfigurationTO(DOIS_LEITORES, "true", FieldType.CHECKBOX, "(usa para catracas com urna)", true));
		geralConfigurations.add(new ConfigurationTO(LEITOR_1, "Entrada e saida_3", FieldType.COMBOBOX,
				"Desativado_0;Somente entrada_1;Somente saida_2;Entrada e saida_3;saida e entrada_4"));
		geralConfigurations.add(new ConfigurationTO(LEITOR_2, "Entrada e saida_3", FieldType.COMBOBOX,
				"Desativado_0;Somente entrada_1;Somente saida_2;Entrada e saida_3;saida e entrada_4"));
		geralConfigurations
				.add(new ConfigurationTO(IDENTIFICACAO_BIOMETRICA, "Sim_1", FieldType.COMBOBOX, "Sim_1;Nao_0"));
		geralConfigurations
	...
	protected void configureInner() {
		...
		this.inner.TipoLeitor = getConfigurationValueAsInteger(TIPO_LEITOR).equals(33) ? 3
				: getConfigurationValueAsInteger(TIPO_LEITOR);
	...
	protected void validarAcesso() {
		try {
			...
			if (inner.BilheteInner.Origem == 1 || inner.BilheteInner.Origem == 2 || inner.BilheteInner.Origem == 3
				...
				if (cartao.startsWith("05000") || cartao.startsWith("005000") || cartao.startsWith("00100")) {
					// Remover "05000" usando substring
				...
				if (ret == Enumeradores.RET_COMANDO_OK) {
					try {
						...
						System.out.println(
								sdf.format(new Date()) + "  VALIDAR ACESSO: Usuario identificado: " + idUsuario);
						if (idUsuario != null) {
			...
			if (VerificationResult.ALLOWED.equals(getVerificationResult())
				...
				if (usaUrna && matchedAthleteAccess != null && matchedAthleteAccess.getOrigemCatraca() != null
	...
	private Long searchTemplate() {
		...
		try {
			...
			if (ret != Enumeradores.RET_COMANDO_OK) {
				EasyInner.EnviarMensagemPadraoOnLine(inner.Numero, 0,
						formatMessage(Utils.getPreference("messageError")));
				return null;
			}
			...
			if (!bsp.IsErrorOccured()) {
				...
				TemplateEntity template = (TemplateEntity) HibernateAccessDataFacade
						.getSingleResultById(TemplateEntity.class, idTemplate.longValue());

				if (template != null) {
					Long idPedestrian = template.getIdPedestreianAccess();
					PedestrianAccessEntity pedestre = pedestrianAccessRepository
							.buscaPedestrePorIdOuIdTemp(idPedestrian);
	...
	protected void enviarConfiguracoesOffline() throws Exception {
		...
		if (ret != easyInner.RET_COMANDO_OK) {
			Main.mainScreen.addEvento(name + ": Nao foi possivel enviar configuracoes offline");
			setStatus(DeviceStatus.DISCONNECTED);
			throw new Exception(name + ": Nao foi possivel enviar configuracoes offline");
		}
	}
	...
	protected void enviarConfiguracaoMudancaOnlineOffline() throws Exception {
		...
		if (inner.Biometrico) {
			...
			EasyInner.DefinirEntradasMudancaOffLineComBiometria(habilitaTeclado, leitor1, leitor2, inner.Verificacao,
					inner.Identificacao);
		} else
			EasyInner.DefinirEntradasMudancaOffLine(habilitaTeclado, inner.ValorLeitor1,
					(inner.DoisLeitores ? inner.ValorLeitor2 : 0), 0);
		...
		EasyInner.DefinirConfiguracaoTecladoOnLine(inner.QtdDigitos,
				(getConfigurationValueAsBoolean(ECOAR_ASTERISCOS) ? 2 : 1), 6, 17);
	...
	private void enviarConfiguracoesOnline() throws Exception {
		...
		if (ret != easyInner.RET_COMANDO_OK) {
			Main.mainScreen.addEvento(name + ": Nao foi possivel enviar configuracoes online");
			setStatus(DeviceStatus.DISCONNECTED);
			throw new Exception(name + ": Nao foi possivel enviar configuracoes online");
		}
	}
	...
	protected void enviarDataHora() throws Exception {
		...
		if (ret != easyInner.RET_COMANDO_OK) {
			Main.mainScreen.addEvento(name + ": Nao foi possivel enviar data e hora");
			setStatus(DeviceStatus.DISCONNECTED);
			throw new Exception(name + ": Nao foi possivel enviar data e hora");
		}
	}
	...
	protected void configurarEntradasOnline() throws Exception {
		...
		if (ret != easyInner.RET_COMANDO_OK) {
			Main.mainScreen.addEvento(name + ": Nao foi possivel enviar configuracoes entradas online");
			setStatus(DeviceStatus.DISCONNECTED);
			throw new Exception(name + ": Nao foi possivel enviar configuracoes entradas online");
		}
	}
...
//			EasyInner.ConfigurarAcionamento2(Enumeradores.NAO_UTILIZADO, 0);
		...
		if (!inner.Biometrico) {
			String leitor1 = inner.ValorLeitor1 == 0 ? "000"
					: (inner.ValorLeitor1 == 1 ? "001"
							: (inner.ValorLeitor1 == 2 ? "010" : (inner.ValorLeitor1 == 3 ? "011" : ("100"))));
			String leitor2 = inner.ValorLeitor2 == 0 ? "000"
					: (inner.ValorLeitor2 == 1 ? "001"
							: (inner.ValorLeitor2 == 2 ? "010" : (inner.ValorLeitor2 == 3 ? "011" : ("100"))));
			 ...
			 * | |
			 * -----------------------------------------------------------------------------
			 * --------------------- | 1 ' Habilita | 000 - Desativa leitor | 000 - Desativa
			 * leitor | 1 ' Habilita | | 0 ' Desabilita | 001 - Leitor só entrada | 001 -
			 * Leitor só entrada | 0 ' Desabilita | | | 010 - Leitor só saida | 010 - Leitor
			 * só saida | | | | 011 - Leitor Entrada e saida | 011 - Leitor Entrada e saida
			 * | | | | 100 - Leitor Entrada e saida | 100 - Leitor Entrada e | | | |
			 * Invertido | saida Invertido | |
			 * -----------------------------------------------------------------------------
			 * ---------------------
			 */
			 ...
			 * identificacao | Verifica��o | Bit fixo | Leitor 1 | Leitor 2 | Teclado | |
			 * '0' | config. | Bio | Bio | '0' | | | | | | bit-a-bit bio | | | | | | |
			 * -----------------------------------------------------------------------------
			 * ------------------------------------------- | 0 | 1-Habilita | 1-Habilita |
			 * 1-Habilita | 0 | 1-Habilita | 1-Habilita | 1-Habilita | | | 0-Desabilita |
	...
	private void startIndexSearchEngine() {
		...
		List<TemplateEntity> templatesList = (List<TemplateEntity>) HibernateAccessDataFacade
				.getResultList(TemplateEntity.class, "TemplateEntity.findAllNaoRemovido");
...
//					addTemplateToIndexSearch(templateEntity);
	...
	public void verificaCadastroNoInner(boolean online, boolean todas, Date data) {
		...
		if (!todas && dataAlteracao != null) {
			...
			System.out.println("Sincroniza total");
			pedestres = (List<PedestrianAccessEntity>) HibernateAccessDataFacade
					.getResultList(PedestrianAccessEntity.class, "PedestrianAccessEntity.findAll");
			todas = true;
		}
	...
	private void processaAdicaoBiometriaLC(TemplateEntity templateEntity) {
		...
		int ret = EasyInner.RequisitarVerificarCadastroUsuarioBio(inner.Numero, 1,
				templateEntity.getPedestrianAccess().getId().toString());
		...
		do {
			...
			Utils.sleep(50);
				...
				&& (System.currentTimeMillis() - inicio) < 5000 && DeviceMode.ENROLLMENT.equals(mode));
		...
		try {
			...
			if (!bsp.IsErrorOccured() && bsp.GetErrorCode() != 0) {
				System.out.println(sdf.format(new Date())
						+ " topdata topdata Erro ao adicionar template na IndexSearchEngine. Erro: "
						+ bsp.GetErrorCode());
			}
