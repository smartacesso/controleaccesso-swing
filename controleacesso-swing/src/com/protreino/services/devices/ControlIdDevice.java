...
public class ControlIdDevice extends Device {
	...
	public void connect(String... args) throws Exception {
		try {
					...
					try {
						while (workerEnabled) {
							...
							try {
								...
								if (caminho.startsWith("/new_user_identified") || caminho.startsWith("/new_card")
										|| caminho.startsWith("/new_user_id_and_password")) {

									// Ex:
									// card_value=0&device_id=935449&duress=0&event=8&identifier_id=1651076864&portal_id=1&user_has_image=0&user_id=1657&user_name=
									...
									System.out.println("\n" + sdf.format(new Date()) + "  RESULTADO DA VALIDACAO: "
											+ verificationResult);

									if (athleteScreen != null)
										athleteScreen.requisicaoPorDigital(null, verificationResult, allowedUserName,
									...
									if (VerificationResult.ALLOWED.equals(verificationResult)) {
										...
										event = 3; // parâmetros de regra de identificacao inválidos
										...
										event = 3; // Nao identificado
										message = verificationResult.getMessage().replace(";", " ");
									}

									System.out.println("\n" + sdf.format(new Date()) + "  MENSAGEM: " + message);

									Boolean userImage = getConfigurationValueAsBoolean("Enviar fotos")
											&& matchedAthleteAccess.getFoto() != null;
									System.out.println(
											"\n" + sdf.format(new Date()) + "  Usuario com foto: " + userImage);

									Response response = new Response(new Result(event, Long.valueOf(idUsuario),
											allowedUserName, message, userImage, actions));
									...
									// caminho =
									// /template_create.fcgi?card_id=&device_id=935449&finger_type=0&size0=2534&temp_num=1&user_id=0
									out.println("HTTP/1.1 200 OK" + "\r\n");
								...
								} else if (caminho.startsWith("/push") || caminho.startsWith("/master_password")
									...
									if (Mode.CONTINGENCY.equals(notificacao.operation_mode.mode_name)) {
										...
										System.out.println("\n\n" + sdf.format(new Date()) + "  MODO DESCONHECIDO: "
												+ notificacao.operation_mode + "\n");
									}
			 ...
			 * 
			 * @Override protected Void doInBackground() throws Exception { while
			 * (synchronizerEnabled) { try { Utils.sleep(30000); sendPhotos(); } catch
			 * (Exception e) { e.printStackTrace(); } finally { Utils.sleep(180000); } }
			 * return null; } }; synchronizer.execute();
			 */

		} catch (SocketTimeoutException ste) {
			throw new SocketTimeoutException("Nao foi possivel conectar na catraca: timeout");
		} catch (Exception e) {
			throw e;
		}
	}
	...
	protected void registraGiro(Response notificacao) {
		...
		String modoOperacao = getConfigurationValue("Modo de operacao");
	...
	public void sendConfiguration() throws Exception {
		...
		for (ConfigurationGroupTO group : configurationGroups) {
			if ("Personalizaçao".equals(group.getName())) {
	...
	protected String decideLadoLiberarCatraca(String _sentidoEntrada) {
		...
		if (quantidadeAcessos == 0) {
			...
			if (deviceEntity.getAttachedDevices() != null && !"".equals(deviceEntity.getAttachedDevices())
	...
	public void processSampleForEnrollment(Object obj) {
		// O processamento das digitais é feito pela catraca
	}
...
//	}
	...
	private List<Integer> createObjects(String object, List<Object> values) throws Exception {
		Object[] retorno = send("http://" + ip + "/create_objects.fcgi?session=" + session,
				new CreateObject(object, values));
		String erro = (String) retorno[0];
		if (erro != null)
			throw new Exception(erro);
		String responseString = (String) retorno[1];
	...
	private Integer modifyObjects(String object, Object values, WhereClause whereClause) throws Exception {
		Object[] retorno = send("http://" + ip + "/modify_objects.fcgi?session=" + session,
				new ModifyObject(object, values, whereClause));
		String erro = (String) retorno[0];
		if (erro != null)
			throw new Exception(erro);
		String responseString = (String) retorno[1];
	...
	private Integer removeObjects(String object, WhereClause whereClause) throws Exception {
		Object[] retorno = send("http://" + ip + "/destroy_objects.fcgi?session=" + session,
				new RemoveObject(object, whereClause));
		String erro = (String) retorno[0];
		if (erro != null)
			throw new Exception(erro);
		String responseString = (String) retorno[1];
	...
	public void sendPhotos() {
		...
		if (listaAcesso != null && !listaAcesso.isEmpty()) {
			...
			// percorre a lista de usuarios e cria pacotes de 50 fotos para enviar pra
			// catraca por vez
			int contador = 0;
	...
	private boolean enviaListaFotos(List<UserImage> listaUsers) {
		try {
			Object[] retorno = send("http://" + ip + "/user_set_image_list.fcgi?session=" + session,
					new SetImageList(listaUsers));
			String erro = (String) retorno[0];
			if (erro != null) {
	...
	public void resetToFactory() throws Exception {
		...
		Object[] retorno = send("http://" + ip + "/reset_to_factory_default.fcgi?session=" + session,
				new Request(true));
		String erro = (String) retorno[0];
		if (erro != null)
			throw new Exception(erro);
	...
	private void verificaDeviceServidor() throws Exception {
		...
		// Caso nao encontre pelo IP, caso tenha trocado de IP, então procura pelo nome
		// ServidorSmartAcesso
		if (serverId == null) {
		...
		if (serverId != null) {
			...
			if (changes == null || changes < 1)
				throw new Exception("Nao foi possivel atualizar os dados do servidor.");
			return;
		}
		...
		if (idsCriados == null || idsCriados.isEmpty())
			throw new Exception("Nao foi possivel criar o servidor.");
	...
	public void createDefaultConfiguration() {
		...
		geralConfigurations.add(new ConfigurationTO("Modo de operação", "Ambos bloqueados_blocked", FieldType.COMBOBOX,
				"Entrada liberada_entrance_open;Saida liberada_exit_open;Ambos bloqueados_blocked;Ambos liberados_both_open"));
		geralConfigurations
				.add(new ConfigurationTO("Tempo de giro", "5000", FieldType.NUMERIC_LIST, "3000;1000;10000")); // inicio;passo;fim
		geralConfigurations
				.add(new ConfigurationTO("Tempo da requisição", "10000", FieldType.NUMERIC_LIST, "2000;1000;20000")); // inicio;passo;fim
	...
	public Set<Integer> getRegisteredUserList() throws Exception {
		/*
		 * Set<Integer> retorno = new HashSet<Integer>(); Response response =
		 * loadObjects("users", null); List<User> listaCatraca = response.users; if
		 * (listaCatraca != null && !listaCatraca.isEmpty()) { for (User user :
		 * listaCatraca) retorno.add(user.id); } return retorno;
		 */
		return null;
	}
	...
	public String cadastrateUser(PedestrianAccessEntity athleteAccessEntity) {
		...
		Boolean usuarioCriadoNestaCatraca = false; // usado para o caso de criar o usuario e nao conseguir criar o
													// template. Nesse caso, apaga o usuario criado
		try {
			final Integer idUsuario = athleteAccessEntity.getId().intValue();
			System.out.println("\n\r" + sdf.format(new Date())
					+ "  ------ CADASTRO DE USUARIO - Criando usuario com id " + idUsuario + "...");
			...
			String erro = (String) retorno[0];
			System.out.println("\n\r" + sdf.format(new Date())
					+ "  ------ CADASTRO DE USUARIO - Erro ao solicitar digital: " + erro);
			if (erro != null) {
				throw new Exception(erro);
			}
			System.out.println("\n\r" + sdf.format(new Date())
					+ "  ------ CADASTRO DE USUARIO - Coleta de digital solicitada! Aguardando recebimento da digital...");
			...
			if (!digitalColetada) {
				System.out.println("\n\r" + sdf.format(new Date())
						+ "  ------ CADASTRO DE USUARIO - Sistema nao recebeu a digital: " + erro);
				throw new Exception("Excedido tempo limite.");
			}
			System.out.println("\n\r" + sdf.format(new Date())
					+ "  ------ CADASTRO DE USUARIO - Digital coletada e recebida! Criando usuario...");
			...
			if (idsCriados == null || idsCriados.isEmpty()) {
				System.out.println("\n\r" + sdf.format(new Date())
						+ "  ------ CADASTRO DE USUARIO - Nao foi possivel criar o usuario: " + erro);
				throw new Exception("Nao foi possivel criar o usuario.");
			}
			usuarioCriadoNestaCatraca = true;
			System.out.println("\n\r" + sdf.format(new Date())
					+ "  ------ CADASTRO DE USUARIO - Usuario criado! Criando template...");
			...
			if (idsCriados == null || idsCriados.isEmpty()) {
				System.out.println("\n\r" + sdf.format(new Date())
						+ "  ------ CADASTRO DE USUARIO - Nao foi possivel criar o template: " + erro);
				excluirUsuario(new PedestrianAccessEntity(athleteAccessEntity.getId()), false);
				usuarioCriadoNestaCatraca = false;
				throw new Exception("Nao foi possivel criar o template.");
			}
			System.out.println("\n\r" + sdf.format(new Date())
					+ "  ------ CADASTRO DE USUARIO - Template criado! Replicando para outras catracas...");
			...
			for (com.protreino.services.devices.Device device : Main.devicesList) {
				if (!device.isTheSame(this) && manufacturer.equals(device.getManufacturer())) {

					System.out.println(
							"\n\r" + sdf.format(new Date()) + "  ------ CADASTRO DE USUARIO - " + device.getName()
									+ " - Status: " + device.isConnected() + " - Espelhar? " + device.isMirrorDevice());

					if (device.isConnected() && device.isMirrorDevice()) {
						ControlIdDevice otherDevice = (ControlIdDevice) device;

						Boolean usuarioEspelhadoNestaCatraca = false; // usado para o caso de criar o usuario e nao
																		// conseguir criar o template. Nesse caso, apaga
																		// o usuario criado
						try {
							// cria o usuario
							values = new ArrayList<Object>();
							...
							if (idsCriados == null || idsCriados.isEmpty()) {
								System.out.println("\n\r" + sdf.format(new Date())
										+ "  ------ CADASTRO DE USUARIO - Nao foi possivel espelhar o usuario em "
										+ device.getName());
								throw new Exception("Erro ao espelhar usuario na catraca " + otherDevice.getName());
							}
							usuarioEspelhadoNestaCatraca = true;
							System.out.println("\n\r" + sdf.format(new Date())
									+ "  ------ CADASTRO DE USUARIO - Usuario replicada! Replicando template...");
							...
							if (idsCriados == null || idsCriados.isEmpty()) {
								// Nao foi possivel criar o template, entao apaga o usuario criado anteriormente
								System.out.println("\n\r" + sdf.format(new Date())
										+ "  ------ CADASTRO DE USUARIO - Nao foi possivel espelhar o template em "
										+ device.getName());
								otherDevice.excluirUsuario(new PedestrianAccessEntity(athleteAccessEntity.getId()),
										false);
								usuarioEspelhadoNestaCatraca = false;
								throw new Exception("Erro ao espelhar template na catraca " + otherDevice.getName());
							}
							System.out.println("\n\r" + sdf.format(new Date())
									+ "  ------ CADASTRO DE USUARIO - Template replicado! Replicando para outras catracas...");

						} catch (Exception e2) {
							System.out.println("\n\r" + sdf.format(new Date())
									+ "  ------ CADASTRO DE USUARIO - Excecao durante o espelhamento em "
	...
	protected enum GiroCatraca {
		EVENT_TURN_LEFT, EVENT_TURN_RIGHT, EVENT_GIVE_UP;
	}
	...
	protected class User {
		...
		private String salt;
		private Integer begin_time; // opcional, Inteiro representando a partir de que data e hora (unix timestamp)
									// o usuario é válido.
		private Integer end_time; // opcional, Inteiro rperesentando até que data e hora (unix timestamp) o
									// usuario é válido.
	...
	protected class Configuration {
		...
		protected Configuration(String hostname, String port, String serverId, Boolean beepEnabled, String tempoGiro,
		...
		protected Configuration(String hostname, String port, String serverId, Boolean beepEnabled, String tempoGiro,
				String requestTimeout, Integer identification_bits, String reader_type, Integer read_interval,
			...
			// this.push_server = new PushConfiguration("");
			this.online_client = new OnlineClient(serverId);
			this.uhf = new UHFConfiguration(identification_bits, reader_type, read_interval, read_interval_diff_tags,
					transmit_power, work_channel, operation_mode, trigger_timeout, trig_idle);
		}
	...
	protected class Result {
		...
		public Result(Integer event, Long user_id, String user_name, String message, Boolean user_image,
				List<Action> actions) {
			this.event = event;
			this.user_id = user_id;
