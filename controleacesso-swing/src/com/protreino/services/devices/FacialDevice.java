...
public class FacialDevice extends Device {
	...
	public FacialDevice(String identifier, List<ConfigurationGroupTO> configurationGroups) {
		...
		} else if ("IP".contains(tipoCamera)) {
	...
	public void createDefaultConfiguration() {
		...
		if ("USB".equals(tipoCamera)) {
			try {
				...
				geralConfigurations.add(new ConfigurationTO("Resolução", resolucaoPadrao + "_" + resolucaoPadrao,
	...
	private String getLowerResolution(String[] arrayResolucoes) {
		...
		for (int i = 1; i < arrayResolucoes.length; i++) {
			...
			if (Integer.valueOf(splited[0]) >= 640 && Integer.valueOf(splited[2]) == 24
	...
	public void sendConfiguration() throws Exception {
		...
		setAthleteScreenConfig(getConfigurationValue("Catraca vinculada") + "%" + "null" + "%" + openAthleteScreenOnInit
				+ "%" + fullScreenAthleteScreen + "%" + focusFieldAthleteScreen + "%");
	}
			...
			protected Void doInBackground() throws Exception {
				...
				while (workerEnabled) {
					...
					try {
						...
						// Para economizar recursos, apenas coleta frame e detecta faces caso seja
						// necessario
						...
						if (FSDK.SaveImageToAWTImage(imageHandle, awtImage,
							...
							// Tenta detectar alguma face com o DetectFace antes de chamar o FeedFrame, por
							// ser um metodo rapido.

							DetectFaceResult resultadoDetectFace = LuxandService.getInstance().detectFace(imageHandle);
							if (resultadoDetectFace == null) {
								continue;
							}

							if (resultadoDetectFace.getResultCode() == FSDK.FSDKE_OK) {

								if(resultadoDetectFace.getFace() != null) {
								...
								if (countToVerifyOnServer == 5) {
									...
									// Verifica se sera necessario tentar identificar alguem ou se apenas exibira a
									// imagem no card
									if (mode.equals(DeviceMode.VERIFICATION)
											&& !getConfigurationValueAsBoolean("Somente para cadastros")) {

										DetectFaceResult resultadoFeedFrame = LuxandService.getInstance().recognize(
												resultadoDetectFace.getFace().getBytes(), imageWidth, imageHeight);

										if (resultadoFeedFrame.getResultCode() == FSDK.FSDKE_OK) {

											drawFace(resultadoFeedFrame.getFace(), gr);

											if (athleteScreen != null && !athleteScreen.isTelaTravada()) {
												...
												if (atualiza && athleteScreen != null) {
													athleteScreen.requisicaoPorFoto(verificationResult, allowedUserName,
															matchedAthleteAccess, 4000);
													allowedUserName = "";
											...
											if (resultadoFeedFrame.getResultCode() == FSDK.FSDKE_USER_NOT_FOUND) {
												...
												if (unsuccessfulTriesInSequence == 3) {
													...
													System.out
															.println("Usuário não cadastrado. Aguardando 5 segundos.");
												}
											} else {
												drawFace(resultadoDetectFace.getFace(), gr);

												if (resultadoDetectFace
														.getResultCode() != FSDK.FSDKE_USER_NOT_LOGGED_IN) {
													// Erro inesperado no FeedFrame, imprime para ver
													System.out.println("Erro no FeedFrame: "
															+ resultadoFeedFrame.getResultDescription());

													System.out.println("id do usário Else  "
															+ resultadoDetectFace.getFace().getIdentifier());
													System.out.println("nome do usário no Else  "
...
//		FSDKCam.FinalizeCapturing();
	...
	public void allowAccess() {
		if (catracaVinculada != null) {
			if (catracaVinculada.isConnected()) {
				...
				Utils.createNotification("Catraca " + catracaVinculada.getName() + " desconectada.",
	...
	public void processSampleForEnrollment(Object obj) {
		...
		// Quando tiver coletado imagens suficientes, entao enviar para o servidor para
		// cadastro
		...
		if (numeroDeImagensCapturadasParaTreinamento == samplesNumberForTraining) {
			...
			DetectFaceResult resultadoFeedFrame = LuxandService.getInstance().enroll(imagesForEnrollment, imageWidth,
					imageHeight, nameToAssign);
			...
			if (resultadoFeedFrame.getResultCode() == FSDK.FSDKE_OK) {
				...
				if (resultadoFeedFrame.getResultCode() == FSDK.FSDKE_USER_ALREADY_REGISTERED) {
					...
					facialDialog
							.cancelCollect("Usuário registrado com o nome " + resultadoFeedFrame.getFace().getName());
				} else if (resultadoFeedFrame.getResultCode() == FSDK.FSDKE_RECOGNITION_FAILED_DURING_ENROLLMENT) {
					facialDialog.cancelCollect(
							"Não foi possível distinguir a pessoa ou usuário cadastrado com outro nome.");
				} else {
...
//			Long luxandIdentifier = Long.valueOf("11");
	...
	private FSDK_VideoFormatInfo.ByValue getVideoFormatFromConfiguration() throws Exception {
		String resolucao = getConfigurationValue("Resolucao");
		if (resolucao == null)
	...
	private void drawFace(Face face, Graphics2D gr, String errorMessage) {
		...
		if (errorMessage.isEmpty()) {
			if (mode.equals(DeviceMode.VERIFICATION) && face.getName() != null) {
				...
				gr.drawString(face.getName(), (int) (facePosition.xc - textRect.getWidth() / 2),
						(int) (top + w + textRect.getHeight()));
			...
			gr.drawString(errorMessage, (int) (facePosition.xc - textRect.getWidth() / 2),
					(int) (top + w + textRect.getHeight()));
		}
