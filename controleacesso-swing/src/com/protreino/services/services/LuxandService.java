...
public class LuxandService {
	...
	public void initializeSDK(String key) {
		try {
			...
			if (ret != FSDK.FSDKE_OK)
				throw new Exception("Nao foi possivel ativar a Luxand FaceSDK. Verifique a chave de licença.");

			ret = FSDK.Initialize();
			if (ret != FSDK.FSDKE_OK)
				throw new Exception("Nao foi possivel inicializar a Luxand FaceSDK.");
			...
			if (ret != FSDK.FSDKE_OK)
				throw new Exception("Nao foi possivel setar os parêmetros de detecção da Luxand FaceSDK.");

			ret = FSDK.SetFaceDetectionThreshold(Integer.valueOf(configurationsMap.get("FaceDetectionThreshold")));
			if (ret != FSDK.FSDKE_OK)
				throw new Exception("Nao foi possivel setar o parâmetro threshold da Luxand FaceSDK.");

			if (Boolean.valueOf(configurationsMap.get("FacilitateRecognizeWithMask"))) {
				int[] erros = new int[10];
				ret = FSDK.SetParameters(
						"FaceDetectionModel=" + PATH_FILE_MASKS_BIN + ";TrimFacesWithUncertainFacialFeatures=false",
						erros);
				if (ret != FSDK.FSDKE_OK)
					throw new Exception("Nao foi possivel setar o parâmetro modelo de detecção de faces.");
			}

			ret = FSDKCam.SetCameraNaming(true); // UseDevicePathAsName
			if (ret != FSDK.FSDKE_OK)
				throw new Exception("Nao foi possivel setar o padrao de nomenclatura da Luxand FaceSDK.");

		} catch (Exception e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, e.toString().substring(0, 150) + "...",
					"Erro ao iniciar Luxand FaceSDK", JOptionPane.PLAIN_MESSAGE);
		}
	}
	...
	public String[] getCameraDescriptionList() {
		...
		if (ret != FSDK.FSDKE_OK) {
			JOptionPane.showMessageDialog(null, "Nao foi possivel retornar lista de nomes das cãmeras.",
		...
		if (cameras.length > 0) {
			...
			JOptionPane.showMessageDialog(null, "Nenhuma cãmera encontrada.", "Erro na Luxand FaceSDK",
...
//	}
	...
	public DetectFaceResult recognize(byte[] image, int imageWidth, int imageHeight) {
		...
		try {
			...
			System.out.println("Luxand Server Response: " + status);
			...
			System.err.println("Nao foi possivel conectar com o servidor");
	...
	public String clearName(long identifier) {
		try {
			...
			HttpConnection httpConnection = new HttpConnection(
					serverRecognizerUrl + "/recognition/identifiers/" + identifier);
			...
			if (status != HttpURLConnection.HTTP_OK) {
				...
				if (status != HttpURLConnection.HTTP_UNAUTHORIZED) {
					errorResponse = "Usuario nao logado no servidor.";
				} else {
	...
	private Map<String, String> getConfigurationsMap() {
		...
		try {
			...
			System.err.println("Nao foi possivel se conectar com o servidor facial.");
