...
public class LuxandService {
	...
	public void initializeSDK(String key) {
		try {
        	...
        	if (ret != FSDK.FSDKE_OK)
	        	throw new Exception("Não foi possível setar os parêmetros de detecção da Luxand FaceSDK.");
	...
	public String[] getCameraDescriptionList() {
		...
		if (ret != FSDK.FSDKE_OK) {
        	JOptionPane.showMessageDialog(null, "Não foi possível retornar lista de nomes das cãmeras.", "Erro na Luxand FaceSDK", JOptionPane.PLAIN_MESSAGE);
        	return new String[0];
        }
        ...
        if (cameras.length > 0) {
        	...
        	JOptionPane.showMessageDialog(null, "Nenhuma cãmera encontrada.", "Erro na Luxand FaceSDK", JOptionPane.PLAIN_MESSAGE);
        }
