...
public class HikiVisionIntegrationService {
	...
	public boolean isFotoUsuarioJaCadastrada(final String deviceId, final String idUser) {
		...
		try (Response response = client.newCall(request).execute();) {
			if (response.isSuccessful()) {
				...
				System.out.println(String.format("Foto do wsuario %s j· cadastrado no device %s: %b", idUser, deviceId,
						isUsuarioCadastrado));
	...
	public boolean isCartaoJaCadastrado(final String deviceId, final String idUser) {
		...
		try (Response response = client.newCall(request).execute();) {
			...
			System.out.println(String.format("Cart„o do usuario %s j· cadastrado no device %s: %b", idUser, deviceId,
