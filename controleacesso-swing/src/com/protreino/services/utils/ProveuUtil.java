		...
		try {
			if (Platform.isWindows()){
				if ("32".equals(Utils.getJvmArchitecture())){
					...
					Utils.createNotification("Catraca Proveu requer JVM 32 bits", NotificationType.BAD);
			}
			else 
				Utils.createNotification("Catraca Proveu requer sistema Windows", NotificationType.BAD);
		}
		catch(Exception e){
			e.printStackTrace();
		}
