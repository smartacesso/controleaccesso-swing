	...
	private void verificaRegrasBusca() {
		...
		if(qrCode != null && !"".equals(qrCode)) {
			...
			if(habilitaQRCode) {
				...
				String tempoQRCode = buscaParametroPeloNome("Permitir acesso via QR Code: Tempo para renova��o do tipo QRCode Din�mico por tempo (em minutos)");
...
//		});
	...
	private void qrCodeDinamicoTempo(JDialog qrCodeDialog, JPanel mainPanel) {
		...
		JLabel mensagemLabel = new JLabel("<html>O QRCode gerado � do tipo din�mico por tempo, renovando-se automaticamente de tempos em tempos,"
				+ "<br/>n�o sendo poss�vel a verifica��o do mesmo fora do App do Pedestre. Voc� deseja excluir esse QRCode?</html>");
		mensagemLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
...
//			}
	...
	private void preencheDadosVisitanteEditando() {
		...
		if("VISITANTE".equals(visitante.getTipo()) 
			...
			//n�o muda valor do cartão adicionado anteriormente
		...
		if(habilitaAppPedestre) {
			...
			senhaLabel.setText("Senha (somente para altera��o)");
		}
...
//		verificar os campoos no confereCatracaOffline
	...
	private void verificaCartaoAcessoRemovido(String cartaoAcesso, Long idPedestre) {
		...
		args.put("ID_PEDESTRE", idPedestre);
//		verifica se o cartão dos pedestres novos est�o sendo vincualdos a pessas j� cadastradas
//		se j�, o cartão � removido da pessoa exluida e j� atribuido ao novo
