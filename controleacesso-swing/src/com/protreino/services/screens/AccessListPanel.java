...
public class AccessListPanel extends PaginedListPanel {
	...
	private String[] columns = { "Codigo", "Cartao", "Nome", "Tipo", "Status", "Regra", "Liberar acesso",
			"Criado por" };
	...
	private void filterList() {
		...
		// Aqui faça a lógica de filtro que pode ser demorada
		...
		new Thread(() -> {
			try {
				...
				// Aqui chama o método que executa a consulta/filtro
				executeFilter();

			} finally {
				SwingUtilities.invokeLater(() -> loading.dispose()); // fecha diálogo na EDT
			}
		}).start();
	}
...
//		fazer isso no hibernate utils
		totalRegistros = HibernateAccessDataFacade.getResultListWithDynamicParamsCount(PedestrianAccessEntity.class,
				construtor, null, null, args);
		calculaTamanhoPaginas();
		listaAcesso = (List<PedestrianAccessEntity>) HibernateAccessDataFacade.getResultListWithDynamicParams(
		...
		if (usuarioDoSistema == null || usuarioDoSistema.isEmpty()) {
			usuarioDoSistema = (List<UserEntity>) HibernateAccessDataFacade.getResultList(UserEntity.class,
					"UserEntity.findAll");
		}
	...
	public void cleanFilter() {
		...
		// getResultListCount(PedestrianAccessEntity.class,
		...
		// "PedestrianAccessEntity.findAllNaoRemovidosOrderedToAccessList",
...
//			SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
	...
	public void updateDateLastSync() {
		if (Objects.nonNull(SyncPedestrianAccessListUseCase.getLastSync())
				&& SyncPedestrianAccessListUseCase.getLastSync() > 0) {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (ccol == 6) { // && pointInsidePrefSize(table, pt)) {
				...
				if (Main.getDefaultDevice() != null && Boolean.TRUE.equals(Main.getDefaultDevice().isConnected())
						&& Boolean.TRUE
								.equals(Main.getDefaultDevice().getConfigurationValueAsBoolean("Bloquear saida"))) {
	...
	class EditVisitanteRenderer extends UrlRenderer {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (ccol == 2) {
				...
				PedestrianAccessEntity visitante = (PedestrianAccessEntity) HibernateAccessDataFacade
						.getUniqueResultWithParams(PedestrianAccessEntity.class, "PedestrianAccessEntity.findById",
								args);
