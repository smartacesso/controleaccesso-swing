	...
	public void procurarAtualizacoes(){
		try {
			...
			if (Double.valueOf(Configurations.VERSION).doubleValue() == versionInfo.getVersion().doubleValue()) {
				...
				int option = JOptionPane.showConfirmDialog(null, panel, "Nova versão dispoível", 
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Não foi possível procurar atualições. Verifique sua conexão e tente novamente.",
            		"Atualização " + Main.nomeAplicacao, JOptionPane.PLAIN_MESSAGE); 
		} finally {
