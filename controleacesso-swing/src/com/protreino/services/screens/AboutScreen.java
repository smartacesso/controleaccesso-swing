...
public class AboutScreen extends JFrame {
	...
	public AboutScreen(boolean abreTela) {
		...
		JButton procurarAtualizacaoButton = new JButton("Procurar atualiza��es");
	...
	public void procurarAtualizacoes(){
		try {
			System.out.println("Procurando atualiza��es...");
			...
			if (responseCode != 200) {
				JOptionPane.showMessageDialog(null, "n�o foi poss�vel procurar atualiza��es. Verifique sua conex�o e tente novamente.",
	            		"Atualiza��o " + Main.nomeAplicacao, JOptionPane.PLAIN_MESSAGE);
				return;
			}
			...
			if (Double.valueOf(Configurations.VERSION).doubleValue() == versionInfo.getVersion().doubleValue()) {
				JOptionPane.showMessageDialog(null, "Voc� j� possui a �ltima Versão instalada!",
	            		"Atualiza��o " + Main.nomeAplicacao, JOptionPane.PLAIN_MESSAGE); 
				...
				JLabel confirmacaoLabel = new JLabel("Deseja instal�-la?");
		    ...
		    public Void doInBackground() {
		    	...
		    	try {
					...
					if (download.getStatus() == 2){
						...
						int option = -1;
						if (Boolean.TRUE.equals(versionInfo.getFullInstaller()))
							option = JOptionPane.showOptionDialog(null, "Download completo! Clique em OK para iniciar a instala��o.", "Atualiza��o pronta!",
									JOptionPane.PLAIN_MESSAGE, JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
			    		else
							option = JOptionPane.showOptionDialog(null, "Download completo! Clique em OK para reiniciar o aplicativo.", "Atualiza��o pronta!",
									JOptionPane.PLAIN_MESSAGE, JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
						if (option == 0) {
							setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
						...
						JOptionPane.showMessageDialog(null, "Ocorreu um erro durante a atualiza��o. Verifique sua conex�o e tente novamente.",
			            		"Atualiza��o " + Main.nomeAplicacao, JOptionPane.PLAIN_MESSAGE);  
					}
			    }
				catch (Exception e){
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				try {
                    ...
                    e.printStackTrace();
                    Utils.createNotification("Erro durante a atualiza��o: " + e.getMessage(), NotificationType.BAD);
                }
			}
		});
		thread.start();
