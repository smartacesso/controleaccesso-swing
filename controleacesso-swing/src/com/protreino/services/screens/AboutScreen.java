	...
	public void procurarAtualizacoes(){
		try {
			...
			if (Double.valueOf(Configurations.VERSION).doubleValue() == versionInfo.getVersion().doubleValue()) {
				...
				JLabel label1= new JLabel("H√° uma nova Vers„o disponÌvel!");
		    ...
		    public Void doInBackground() {
		    	...
		    	try {
					...
					if (download.getStatus() == 2){
						...
						JOptionPane.showMessageDialog(null, "Ocorreu um erro durante a atualizaÁ„o. Verifique sua conex√£o e tente novamente.",
			            		"Atualiza√ß√£o " + Main.nomeAplicacao, JOptionPane.PLAIN_MESSAGE);  
					}
			    }
				catch (Exception e){
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				try {
                    ...
                    e.printStackTrace();
                    Utils.createNotification("Erro durante a atualizaÁ„o: " + e.getMessage(), NotificationType.BAD);
                }
			}
		});
		thread.start();
