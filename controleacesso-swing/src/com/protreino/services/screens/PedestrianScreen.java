...
public class PedestrianScreen extends JFrame {
	...
	private int imageHeight; // sera calculada de acordo com o tamanho da imagem da camera para manter a
								// proporcao
	...
	public PedestrianScreen(Device readerDevice, Device ticketGateDevice_, String athleteScreenConfig) {
		...
		setTitle(Main.nomeAplicacao + " Controle de acesso "
				+ (this.readerDevice.getName() != null ? " - " + this.readerDevice.getName() : ""));
		...
		leftContainer.setBorder(
				new CompoundBorder(new MatteBorder(0, 0, 0, 1, firstColor), new EmptyBorder(10, 20, 10, 20)));
		...
		JLabel insiraCodigoLabel = new JLabel("Digite seu Codigo");
		...
		if (readerDevice.isLeitorBiometrico() || readerDevice.isCatraca()) {
...
//				catracasCombobox.addItem(new SelectItem(commDevice.getName(), commDevice));
					...
					try {
						...
						if (readerDevice != null) {
							leitorLabel.setText(readerDevice.getName() + " - "
									+ (DeviceStatus.CONNECTED.equals(readerDevice.getStatus()) ? "conectado"
											: "desconectado"));
							leitorLabel.setForeground(
									DeviceStatus.CONNECTED.equals(readerDevice.getStatus()) ? secondColor : Color.RED);
						} else {
						...
						if (focusFieldAthleteScreen && !jPopup.isVisible() && !codigoTextField.isFocusOwner()
								&& catracasCombobox != null && !catracasCombobox.isPopupVisible())
	...
	/**
	 * Informa se a tela esta travada, impossibilitando assim novos acessos que a
	 * tela seja limpa
	 * 
	 * @param message
	 */
	...
	public void digitalObtida(DPFPSample sample) {
		if (sample != null) {
			...
			Image sampleImage = DPFPGlobal.getSampleConversionFactory().createImage(sample).getScaledInstance(160, 201,
					Image.SCALE_SMOOTH);
			if (sampleImage != null) {
	...
	/**
	 * Metodo chamado apos o leitor/catraca processar uma requisicao de acesso por
	 * digital. O metodo extrai a imagem da amostra caso haja, e exibe as mensagens
	 * na tela de acordo com o resultado da verificacao recebido
	 * 
	 * @param sample
	 ...
	 */
	public void requisicaoPorDigital(DPFPSample sample, VerificationResult verificationResult, String allowedUserName,
			PedestrianAccessEntity matchedAthleteAccess) {
		try {
			...
			if (sample != null) {
				Image sampleImage = DPFPGlobal.getSampleConversionFactory().createImage(sample).getScaledInstance(160,
						201, Image.SCALE_SMOOTH);
				if (sampleImage != null) {
					BufferedImage sampleCombined = new BufferedImage(160, 201, BufferedImage.TYPE_INT_ARGB);
					Graphics g = sampleCombined.getGraphics();
					g.drawImage(VerificationResult.ALLOWED.equals(resultadoVerificacao)
							|| VerificationResult.TOLERANCE_PERIOD.equals(resultadoVerificacao)
									? digitalVerificadaBordaImage
									: digitalNaoVerificadaBordaImage,
							0, 0, null);
					g.drawImage(sampleImage, 0, 0, null);
					g.drawImage(VerificationResult.ALLOWED.equals(resultadoVerificacao)
							|| VerificationResult.TOLERANCE_PERIOD.equals(resultadoVerificacao)
	...
	/**
	 * Metodo chamado apos processar uma requisicao de acesso por reconhecimento
	 * facial O metodo exibe as mensagens na tela de acordo com o resultado da
	 * verificacao recebido
	 * 
	 * @param sample
	 * @param verificationResult
	 * @param allowedUserName
	...
	public void requisicaoPorFoto(VerificationResult verificationResult, String allowedUserName,
		...
		try {
			...
			if (matchedAthleteAccess != null) {
				if ("".equals(this.allowedUserName) && matchedAthleteAccess.getName() != null) {
	...
	/**
	 * Metodo chamado apos o leitor/catraca processar uma requisicao de acesso por
	 * Codigo. O metodo exibe as mensagens na tela de acordo com o resultado da
	 * verificacao recebido
	 * 
	 * @param sample
	 * @param verificationResult
	 ...
	 * @param matchedAthleteAccess
	 */
	public void requisicaoPorCodigo(VerificationResult verificationResult, String allowedUserName,
			PedestrianAccessEntity matchedAthleteAccess) {
		try {
			setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
...
//			Object[] retorno = HibernateUtil.processAccessRequest(codigoTextField.getText(), readerDevice.getLocation(), false);
			Object[] retorno = processAccessRequestUseCase.processAccessRequest(codigoTextField.getText(),
					readerDevice.getName(), 1, readerDevice.getName(), false, true,
			...
			e.printStackTrace();
			setErroCodigo("Erro ao procurar Codigo.");
	...
	// Faz a verificacao de acordo com o codigo digitado
	private void processarRequisicaoPorCodigoFacial(String cartaoAcesso) {
		try {
			setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
			Object[] retorno = processAccessRequestUseCase.processAccessRequest(cartaoAcesso,
					null, 2, readerDevice.getName(), false, true,false);
	...
	private void verificaSePossuiMensagem(PedestrianAccessEntity matchedAthleteAccess) {
		if (matchedAthleteAccess.getMensagens() != null && !matchedAthleteAccess.getMensagens().isEmpty()) {
		...
		limpador = new SwingWorker<Void, Void>() {
			...
			protected Void doInBackground() throws Exception {
				...
				for (int i = 0; i < contador; i++) {
					// aguarda o tempo necessario, mas sempre verificando se precisa ser cancelado
					// ou se pode continuar
					if (cancelarLimpador)
						break;
					Utils.sleep(1);
				}
		...
		try {
			...
			case ERROR:
				if ("CODIGO".equals(tipoAcesso))
					messageErroCodigoLabel.setText("Erro ao procurar Codigo.");
...
//	    			}
				...
				if (ticketGateDevice != null) {
					if (readerDevice.isLeitorBiometrico() || readerDevice.isCamera() || "CODIGO".equals(tipoAcesso)) {
...
//					statusLabel.setText("Sua mensalidade está vencida. Você ainda tem " + diasRestantes + " para o pagamento.");
			...
			e.printStackTrace();
			if ("CODIGO".equals(tipoAcesso))
				messageErroCodigoLabel.setText("Erro ao procurar Codigo. " + e.getMessage());
			else if ("DIGITAL".equals(tipoAcesso))
	...
	private void loadImages() {
		try {
			...
			logoImageIcon = new ImageIcon(toolkit.getImage(
					Main.class.getResource(Configurations.IMAGE_FOLDER + Main.customImageFolder + "logo_grd001.png")));
			configImageIcon = new ImageIcon(toolkit.getImage(
					Main.class.getResource(Configurations.IMAGE_FOLDER + Main.customImageFolder + "gear.png")));
			naoPermitidoImageIcon = new ImageIcon(
					toolkit.getImage(Main.class.getResource(Configurations.IMAGE_FOLDER + "comuns/nao_permitido.png")));
			vazioImageIcon = new ImageIcon(
					toolkit.getImage(Main.class.getResource(Configurations.IMAGE_FOLDER + "comuns/vazio.png")));
			digitalNaoVerificadaBordaImage = toolkit.getImage(
					Main.class.getResource(Configurations.IMAGE_FOLDER + "comuns/digital_nao_verificada_borda.png"));
			digitalGenericaNegadaImageIcon = new ImageIcon(toolkit.getImage(
					Main.class.getResource(Configurations.IMAGE_FOLDER + "comuns/digital_generica_negada.png")));

			Image semDigitalImage = toolkit
					.getImage(Main.class.getResource(Configurations.IMAGE_FOLDER + "comuns/sem_digital.png"));
			semDigitalImageIcon = new ImageIcon(semDigitalImage);

			BufferedImage mascara = ImageIO
					.read(Main.class.getResource(Configurations.IMAGE_FOLDER + "comuns/permitido_mask.png"));
			permitidoImageIcon = new ImageIcon(Utils.paintImage(mascara, secondColor, 256, 256));

			mascara = ImageIO
					.read(Main.class.getResource(Configurations.IMAGE_FOLDER + "comuns/digital_generica_mask.png"));
			digitalGenericaPermitidaImageIcon = new ImageIcon(Utils.paintImage(mascara, secondColor, 160, 201));

			mascara = ImageIO
					.read(Main.class.getResource(Configurations.IMAGE_FOLDER + "comuns/digital_borda_mask.png"));
	...
	private class BackgroundContentPane extends JPanel {
		...
		private Image getScaledImage(Image srcImg, int w, int h, String extension) {
			BufferedImage resizedImg = new BufferedImage(w, h,
					"png".equals(extension) ? BufferedImage.TYPE_INT_ARGB : BufferedImage.OPAQUE);
			Graphics2D g2 = resizedImg.createGraphics();
			g2.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
	...
	public void requisicaoHivisionServer(String cartaoAcesso, int tempoDeEspera) {
		...
		try {
			...
			processarRequisicaoPorCodigoFacial(cartaoAcesso);

			statusLabel.setText(
					"<html><div style='text-align: center;'>" + Utils.getPreference("messageAllowedAthleteScreen")
							+ "<br>" + matchedAthleteAccess.getName() + "</div></html>");

			switch (resultadoVerificacao) {
			case NOT_FOUND:
				statusLabel.setText(
							"<html><div style='text-align: center;'>" + "Face nao reconhecida ou mal posicionada."
							+ "<br>" + matchedAthleteAccess.getName() + "</div></html>");
			...
			case NOT_ALLOWED:
				statusLabel.setText(
						"<html><div style='text-align: center;'>" + Utils.getPreference("messageNotAllowedAthleteScreen")
						+ "<br>" + matchedAthleteAccess.getName() + "</div></html>");
				resultadoLabel.setIcon(naoPermitidoImageIcon);
				Utils.sleep(tempoDeEspera);
				cleanScreen(true, true);
			...
			case NOT_ALLOWED_FACE_REQUIRED:
				statusLabel.setText(
						"<html><div style='text-align: center;'>" + Utils.getPreference("messageNotAllowedFaceRequired")
						+ "<br>" + matchedAthleteAccess.getName() + "</div></html>");
			...
			case NOT_ALLOWED_TODAY:
				statusLabel.setText(
						"<html><div style='text-align: center;'>" + Utils.getPreference("messageNotAllowedTodayAthleteScreen")
						+ "<br>" + matchedAthleteAccess.getName() + "</div></html>");
				resultadoLabel.setIcon(naoPermitidoImageIcon);
				Utils.sleep(tempoDeEspera);
				cleanScreen(true, true);
			...
			case ALLOWED_ONLY_ONCE:
				statusLabel.setText(
						"<html><div style='text-align: center;'>" + Utils.getPreference("messageAllowedOnlyOnceAthleteScreen")
						+ "<br>" + matchedAthleteAccess.getName() + "</div></html>");
				resultadoLabel.setIcon(naoPermitidoImageIcon);
				Utils.sleep(tempoDeEspera);
				cleanScreen(true, true);
			...
			case NOT_ALLOWED_NOW:
				statusLabel.setText(
						"<html><div style='text-align: right;'>" + Utils.getPreference("messageNotAllowedNowAthleteScreen")
						+ "<br>" + matchedAthleteAccess.getName() + "</div></html>");
				resultadoLabel.setIcon(naoPermitidoImageIcon);
				Utils.sleep(tempoDeEspera);
				cleanScreen(true, true);
				break;
			case NOT_ALLOWED_NO_CREDITS:
				statusLabel.setText(
						"<html><div style='text-align: center;'>" + "Não permitido, sem creditos"
						+ "<br>" + matchedAthleteAccess.getName() + "</div></html>");
				resultadoLabel.setIcon(naoPermitidoImageIcon);
				Utils.sleep(tempoDeEspera);
				cleanScreen(true, true);
